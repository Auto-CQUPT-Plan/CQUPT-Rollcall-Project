...
### 1.6 获取共享签到接收状态 (新增)
*   **接口**: `GET /pause_shared`
*   **功能**: 获取当前边缘服务端是否暂停接收来自中心服务端的共享签到任务。
*   **响应示例**:
    ```json
    { "pause": false }
    ```
...
### 1.7 切换共享签到接收状态 (新增)
*   **接口**: `POST /pause_shared`
*   **功能**: 设置边缘服务端是否暂停接收来自中心服务端的共享签到任务。开启暂停后，通过 WebSocket 接收到的 `rollcall_share` 消息将被忽略，但依然允许本地用户主动通过 HTTP API 提交。
*   **请求体**:
    ```json
    { "pause": true }
    ```
